 ...
 *
 * Canvas geometry: the scene is anchored at 640,268 and runs 880x264. Slabs are
 * 158x264 before scale and step 178 apart — measured off the reference by
...
export function CategoryDeck({
  ...
  return (
    <>
      ...
      <div
        aria-hidden="true"
        className="ph-wire pointer-events-none absolute hidden lg:block lg:left-[calc(555*var(--s))] lg:top-[calc(471*var(--s))] lg:z-[1] lg:h-px lg:w-[calc(1021*var(--s))] lg:bg-[rgba(17,17,17,0.14)]"
      />
      <span
        aria-hidden="true"
      ...
      <span
        aria-hidden="true"
        className="ph-wire pointer-events-none absolute hidden rounded-full bg-[hsl(var(--yellow))] shadow-[0_0_12px_3px_rgba(246,242,60,0.6)] lg:block lg:left-[calc(1571.5*var(--s))] lg:top-[calc(466.5*var(--s))] lg:z-[1] lg:h-[calc(9*var(--s))] lg:w-[calc(9*var(--s))]"
      />
...
function DeckCard({
  ...
  return (
    <button
      ...
      className={`ph-deck-card deck-slab w-[142px] flex-none snap-start rounded-[22px] text-center lg:absolute lg:left-[var(--card-x)] lg:top-0 lg:h-[calc(264*var(--s))] lg:w-[calc(158*var(--s))] lg:rounded-[calc(24*var(--s))] lg:[transform-origin:50%_50%] lg:[transform:translate(calc(var(--px,0)*3px),calc(var(--py,0)*3px))_rotateY(-14deg)_scale(var(--card-scale))] ${
        active ? "is-active" : ""
      }`}
    >
      ...
      <span className="deck-face glass-card-inner flex h-full w-full flex-col items-center justify-center px-4 py-10 lg:px-0 lg:py-0">
        <Icon
          className="h-[32px] w-[32px] text-[hsl(var(--ink-1))] lg:h-[calc(30*var(--s))] lg:w-[calc(30*var(--s))]"
          strokeWidth={1.7}
          aria-hidden="true"
        />
        <span className="mt-4 block text-[21px] font-extrabold tracking-[-0.02em] lg:mt-[calc(18*var(--s))] lg:text-[calc(21*var(--s))]">
          {category.label}
        </span>
        <span className="mt-1.5 block whitespace-nowrap text-[12px] text-[hsl(var(--ink-3))] lg:mt-[calc(7*var(--s))] lg:text-[calc(11*var(--s))]">
